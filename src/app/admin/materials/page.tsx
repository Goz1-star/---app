    ...
    <AdminShell>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
        <Card>
          ...
          <form action={createMaterialAction} className="mt-4 grid gap-3" encType="multipart/form-data">
            ...
            <p className="text-xs text-slate-500">可上传 PDF、Word、Markdown、图片、压缩包或代码文件；代码文件最大 200MB。</p>
          ...
          {materials.map((item) => (
            <Card key={item.id}>
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                {item.url ? <a href={item.url} target="_blank" className="rounded-full bg-slate-950 px-4 py-2 text-sm font-semibold text-white">打开链接</a> : null}
              </div>
              {item.files.length ? (
                <div className="mt-4 space-y-2">
                  {item.files.map((file) => (
                    <div key={file.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-slate-50 p-3 text-sm text-slate-600">
                      <span>{file.originalName} · {formatFileSize(file.size)}</span>
                      <div className="flex gap-2">
                        {file.previewable ? <a href={`/admin/files/${file.id}/preview`} className="font-semibold text-brand-700">预览</a> : null}
